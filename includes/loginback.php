        ...
        if (isset($_POST['login_btn'])) {
            ...
            if (isset($_POST['email'], $_POST['password'])) {
                ...
                if ($username == " " || $password == " ") {
                    ...
                    $query = "select pass,status,vkey,email,custID from customer where email=?";
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    if (password_verify($password, $row['pass'])) {
                                        if ($row['status'] == 2) {
                                            ...
                                            header("Location:../supPages/resetpass.php?vkey=" . $row['vkey']);
                                        } else if ($row['status'] == 0) {
                                            header("Location:../supPages/regThanks.php?vkey=" . $row['vkey'] . "&email=" . $row['email']);
                                        }
                                    } else {
